...
export default function NavBar({ onNavClick, activeSection, onGetStartedClick }: NavBarProps) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-border px-4 sm:px-6 md:px-8 transition-all duration-300" style={{ padding: '4px 1rem' }}>
            ...
            if (item.path) {
              return (
                <Link
                  key={item.value}
                  to={item.path}
                  className={`font-sans text-[11px] xl:text-xs font-medium transition-all duration-300 relative py-1 hover:text-primary ${
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`lg:hidden fixed inset-0 top-[65px] sm:top-[77px] z-40 bg-white border-t border-border transition-all duration-300 ease-in-out transform ${
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  key={item.value}
                  onClick={() => handleItemClick(item.value)}
                  className={`flex items-center justify-between py-3 px-4 rounded-lg font-sans text-sm font-semibold text-left transition-all ${
                    isActive 
